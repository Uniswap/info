    ...
    if (!chartCreated) {
      ...
      function setLastBarText() {
        toolTip.innerHTML = base
          ? `<div style="font-size: 22px; margin: 4px 0px 4px 36px; color: ${textColor}; font-weight: bold">` + valueFormatter(base) + '</div>'
          : ''
      }
      setLastBarText()
      ...
      chart.subscribeCrosshairMove(function (param) {
        if (
          ...
          toolTip.innerHTML =
            `<div style="font-size: 22px; margin: 4px 0px 4px 36px; color: ${textColor};">` +
            `<span style="font-weight: bold; font-size: 20px">${valueFormatter(price)}</span>` +
            `<span style="font-size: 14px; margin: 4px 6px; color: ${textColor}">` +
            time +
            ' UTC' +
            '</span>' +
            '</div>'
